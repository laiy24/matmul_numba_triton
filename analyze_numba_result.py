...
def plot_1_level_tiling_effect(df, N, op_type, parallel, transposed, title_prefix):
    """
    Plots Cache Miss Rates and Total Cycles vs. Block Size (B1) for 1-level tiling.
    Filters for B2 == 0.
...
def plot_2_level_tiling_effect(df, N, op_type, parallel, transposed, title_prefix):
    """
    Plots Cache Miss Rates and Total Cycles vs. Block Size (B2) for 2-level tiling.
    - Grouped by B1.
...
def get_best_run(data, filters):
    """Helper to filter dataframe and find the row with minimum cycles."""
    subset = data.copy()
    ...
    # Find and return the row with the minimum cycles
    return subset.loc[subset['cycles'].idxmin()]
...
def plot_parallel_comparison_bar(df, N):
    ...
    sns.barplot(data=plot_df, x='Base Task', y='cycles', hue='Type', ax=ax)
...
def plot_transpose_comparison_bar(df, N):
    ...
    sns.barplot(data=plot_df, x='Base Task', y='cycles', hue='Access Pattern', ax=ax)
...
def main():
    ...
    # get the lowest cycle config for matmul at size N_to_plot but not numpy_mul
    matmul_df = df[(df['op_type'] == 'mul') & (df['N'] == N_to_plot) & (df['base_impl'] != 'numpy_mul')]
    best_row = matmul_df.loc[matmul_df['cycles'].idxmin()]
    print(f"\nBest MatMul Configuration at N={N_to_plot}:")
    print(best_row[['benchmark_name', 'B1', 'B2', 'is_parallel', 'is_transposed', 'cycles']])   
